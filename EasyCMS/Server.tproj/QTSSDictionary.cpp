	...
	bool isInternal)
{
	...
	*outValueLen = theAttrs[theMapIndex].fAttributeData.Len;
	...
	if ((theMap->GetAttrFunction(theMapIndex) != NULL) && ((cacheable && (*outValueLen == 0)) || !cacheable))
	{
		...
		theBuffer = static_cast<char*>(theMap->GetAttrFunction(theMapIndex)(this, outValueLen));
	...
	*outValueBuffer = theBuffer;
	...
	{
		char** string = reinterpret_cast<char**>(theBuffer);
	...
	UInt32 inFlags)
{
	...
	if (dataType == qtssAttrDataTypeCharArray)
	{
		...
		if ((numValues == 1) && (inIndex == 1))
		{
			...
			*reinterpret_cast<char**>(theAttrs[theMapIndex].fAttributeData.Ptr) = temp;
...
QTSS_Error QTSSDictionary::RemoveValue(QTSS_AttributeID inAttrID, UInt32 inIndex, UInt32 inFlags)
{
	...
	if (theAttrs[theMapIndex].fIsDynamicDictionary)
	{
		...
		QTSSDictionary* dict = *reinterpret_cast<QTSSDictionary**>(theAttrs[theMapIndex].fAttributeData.Ptr + (theValueLen * inIndex));
		delete dict;
	}
	...
	if ((dataType == qtssAttrDataTypeCharArray) && (numValues > 1))
	{
		...
		char* str = *reinterpret_cast<char**>(theAttrs[theMapIndex].fAttributeData.Ptr + (theValueLen * inIndex));
		delete[] str;
	}
	...
	if ((dataType == qtssAttrDataTypeCharArray) && (theAttrs[theMapIndex].fNumAttributes == 1))
	{
		...
		char* str = *reinterpret_cast<char**>(theAttrs[theMapIndex].fAttributeData.Ptr);
		delete theAttrs[theMapIndex].fAttributeData.Ptr;
	...
	UInt32 inBufferLen)
{
	Assert(inAttrID >= 0);
	Assert(fMap);
	Assert(static_cast<UInt32>(inAttrID) < fMap->GetNumAttrs());
	fAttributes[inAttrID].fAttributeData.Ptr = static_cast<char*>(inValueBuffer);
...
UInt32  QTSSDictionaryMap::GetMapIndex(QTSS_ObjectType inType)
{
	...
	switch (inType)
	{
	case qtssHTTPSessionObjectType:     return kHTTPSessionDictIndex;//4
	case qtssServerObjectType:          return kServerDictIndex;//0
	case qtssPrefsObjectType:           return kPrefsDictIndex;//1
	case qtssTextMessagesObjectType:    return kTextMessagesDictIndex;//2
	case qtssFileObjectType:            return kFileDictIndex;//7
...
QTSS_ObjectType QTSSDictionaryMap::CreateNewMap()
{
	...
	QTSS_ObjectType result = static_cast<QTSS_ObjectType>(sNextDynamicMap);
	sNextDynamicMap++;
